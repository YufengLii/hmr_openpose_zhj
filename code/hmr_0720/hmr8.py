...
def capture_image(pipe_img):
    ...
    while True:    
        ...
        if capture_count == 300 :
            print('Capture FPS:', 1.0 / ((time.time() - capture_time)/ 100.0))
            capture_count = 0
      	    capture_time = time.time() 
    return 0


def detection(pipe_img,pipe_center,pipe_scale,pipe_img_2):
    ...
    while True: 
        ...
        #cv2.imwrite('kp.jpg', bodyKeypoints_img)
...
p2 = multiprocessing.Process(target=detection, args=(pipe_img[1],pipe_center[0],pipe_scale[0],pipe_img_2[0],)) 
p3 = multiprocessing.Process(target=rec_human, args=(pipe_img_2[1],pipe_center[1],pipe_scale[1],)) 
